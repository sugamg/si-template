...
def register_best_model(model_name, experiment_name, metric, order_by="ASC", model_artifact_name="model"):
    """
    ...
    best_run = client.search_runs(experiment_id, order_by=[f"metrics.{metric} {order_by}"])[0]
...
#     best_run_from_parent_run = client.search_runs(experiment_id, filter_string=f"tags.mlflow.parentRunId = '{parent_run_id}'", order_by=[f"metrics.{metric} {order_by}"])[0]
    ...
    best_model_uri = f"runs:/{best_run.info.run_id}/{model_artifact_name}"
